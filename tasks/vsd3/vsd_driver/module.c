    ...
    char __user *read_user_buf, size_t read_size, loff_t *fpos)
{
     ...
     * 1. Driver executes wait_event_interruptible
     * 2. Interrupt comes and control-flow goes to
     *    vsd_dev_dma_op_complete_tsk_func
     * 3. vsd_dev_dma_op_complete_tsk_func returns driver into scheduler's queue
     * 4. Scheduler switches to the driver and the driver frees kernel buffer
     * 5. Scheduler switches to the device and the device writes to freed buffer
     */
